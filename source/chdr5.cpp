...
void CHDR5::SetFile(CFile* pfile) {
	Init();
	m_pFile = pfile;
}

int CHDR5::LoadBuf(LONGLONG lpos) {
	if (m_pFile == NULL) return 16051500;
	lpos = m_pFile->Seek(lpos, CFile::begin);
	m_iBufferLength = m_pFile->Read(m_pcBuffer, sizeof(char) * CHDR5_BUFSIZE);
//	if (m_pFile->Read(m_pcBuffer, sizeof(char) * CHDR5_BUFSIZE) != sizeof(char) * CHDR5_BUFSIZE)
//		{AfxMessageBox("16051501"); return 16051501;}
	m_lBufStart = lpos;
	return 0;
}

unsigned char* CHDR5::GetPointer(LONGLONG lpos, unsigned int ibyte) {
	if ((m_lBufStart < 0)||(lpos + ibyte > m_lBufStart + CHDR5_BUFSIZE)||(lpos < m_lBufStart)) {
...
__int64 CHDR5::GetOffsetParam(unsigned char* pcPnt, __int64 lpos) {
	switch (m_iSzOffset) {
...
__int64 CHDR5::GetLengthParam(unsigned char* pcPnt, __int64 lpos) {
	switch (m_iSzLength) {
		case 1: {return *((unsigned char*)(&(pcPnt[lpos])));}
		case 2: {return *((unsigned short*)(&(pcPnt[lpos])));}
		case 4: {return *((unsigned int*)(&(pcPnt[lpos])));}
...
int CHDR5::ReadSuperBlock(CString* psMsg) {
...
int CHDR5::FindChildSymbol(CString sSymbol, int iEntry, CString* psMsg) {
...
int CHDR5::GetDataObjHeader(CString* psMsg) {
		...
		switch (uiHeaderMsgType) {
					...
					if (m_uiDataElementSize == 4) {
						if ((ucClassBitField0 == 32)&&(ucClassBitField8 == 31)&&
							...
							(ucMantissaLocation == 0)&&(ucMantissaSize == 23)&&(uiExpBias == 127)) {
								m_uiDataElementType = CHDR5_DATAELEMTYPE_FLOAT;
						} else {
							ierr = 16052122; break;
						}
						...
						if ((ucClassBitField0 == 32)&&(ucClassBitField8 == 63)&&
							...
							(ucMantissaLocation == 0)&&(ucMantissaSize == 52)&&(uiExpBias == 1023)) {
								m_uiDataElementType = CHDR5_DATAELEMTYPE_DOUBLE;
						} else {
							ierr = 16052123; break;
						}
					} else {
					...
					if (m_uiDataElementSize == 2) {
						if ((ucClassBitField0 == 0)&&(ucClassBitField8 == 0)&&
							(ucClassBitField16 == 0)&&(uiBitOffset == 0)&&(uiBitPrecision == 16)) {
								m_uiDataElementType = CHDR5_DATAELEMTYPE_SHORT;
						} else {
							ierr = 16061602; break;
						}
					} else {
			...
			case 8: {
				...
				if (ucLayoutClass == 1) {//contiguous
					...
					if (ucDimension > CHDR5_MAXDIM) {ierr = 16061621; break;}
...
int CHDR5::ReadFrame(unsigned int uiFrame, int* piData, CString* psMsg) {
	...
	if (psMsg) {
		...
		line.Format("Frame= %d\r\n", uiFrame); *psMsg += line;
		if (m_uiDataElementType == CHDR5_DATAELEMTYPE_FLOAT) {*psMsg += "DataFormat= float\r\n";}
		else if (m_uiDataElementType == CHDR5_DATAELEMTYPE_DOUBLE) {*psMsg += "DataFormat= double\r\n";}
		else if (m_uiDataElementType == CHDR5_DATAELEMTYPE_SHORT) {*psMsg += "DataFormat= short\r\n";}
		else {*psMsg += "DataFormat= unknown\r\n"; return 16052204;}
	}
	if (m_ucDataDim != 3) return 16052201;
	if (piData == NULL) return 16052202;
	if (m_plDataSize[2] > CHDR5_BUFSIZE) return 16052203;
...
TErr CHDR5::ReadStrip(short* psBuf, unsigned int uiFrame, int iLine, int iMultiplex) {
	//
	TErr ierr = 0;
	if (m_ucDataDim != 3) return 16052711;
	if (psBuf == NULL) return 16052712;
	if (m_plDataSize[2] > CHDR5_BUFSIZE) return 16052713;
...
int CHDR5::ReadTheta(float* pfData, CString* psMsg) {
	...
	if (m_plDataSize[0] > CHDR5_BUFSIZE) return 16052623;
	int idx = 0;
...
int CHDR5::GetBTreeNodeParams(__int64 lBTreeAddr, char* pcNodeType, char* pcNodeLevel, int* piEntriesUsed,
...
int CHDR5::GetSymbolTable(__int64 lChildPointer, CString* psSymbol, int* piEntry, 
...
CString CHDR5::GetStringFromLocalHeap(__int64 lLinkNameOffset, CString* psMsg) {
...
void CHDR5::Debug() {
	...
	CHDR5 hdr5;
	hdr5.SetFile(&fhdf5);
	int ierr = hdr5.ReadSuperBlock(&msg);
	if (ierr) {line.Format("Err %d", ierr); AfxMessageBox(line);}
	ierr = hdr5.FindChildSymbol("exchange", &msg);
	if (ierr) {line.Format("Err %d", ierr); AfxMessageBox(line);}
	hdr5.MoveToChildTree();
	ierr = hdr5.FindChildSymbol("data", &msg);
	if (ierr) {line.Format("Err %d", ierr); AfxMessageBox(line);}
	ierr = hdr5.GetDataObjHeader(&msg);
	if (ierr) {line.Format("Err %d", ierr); AfxMessageBox(line);}
	ierr = hdr5.GetDataFloat(&msg);
	if (ierr) {line.Format("Err %d", ierr); AfxMessageBox(line);}
	ierr = hdr5.FindChildSymbol("data_dark", &msg);
	if (ierr) {line.Format("Err %d", ierr); AfxMessageBox(line);}
	ierr = hdr5.GetDataObjHeader(&msg);
	if (ierr) {line.Format("Err %d", ierr); AfxMessageBox(line);}
	ierr = hdr5.GetDataFloat(&msg);
