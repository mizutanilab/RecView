...
void CudaReconstMemFree(RECONST_INFO* ri) {
	...
	//130923 if (ri->d_p != NULL) cutilSafeCall(cudaFree(ri->d_p));
	ri->d_p = NULL;
	ri->max_d_p = 0;
	if (ri->d_filt != NULL) cudaFree(ri->d_filt);
	//130923 if (ri->d_filt != NULL) cutilSafeCall(cudaFree(ri->d_filt));
	ri->d_filt = NULL;
	ri->max_d_filt = 0;
	if (ri->d_strip != NULL) cudaFree(ri->d_strip);
	//130923 if (ri->d_strip != NULL) cutilSafeCall(cudaFree(ri->d_strip));
	ri->d_strip = NULL;
	ri->max_d_strip = 0;
	if (ri->d_igp != NULL) cudaFree(ri->d_igp);
	//130923 if (ri->d_igp != NULL) cutilSafeCall(cudaFree(ri->d_igp));
	ri->d_igp = NULL;
	ri->max_d_igp = 0;
