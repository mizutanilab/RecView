...
## Release notes
The binary folder contains 64-bit (x64) and 32-bit (x86) executables. They were complied using Visual Studio 2017 C++ and CUDA Toolkit 10.0. If your PC is running 64-bit Windows and has an NVIDIA Tesla, GeForce, Quadro, or Tegra GPU processor with 'compute capability' of 3.0 or higher (Kepler or later), the reonstruction calculation can be accelerated with the GPU. The former version including 32-bit CUDA and ATI executables (along with their source code) is placed in the 'vs2008' folder. 
