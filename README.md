...
We now have a plan to move to Visual Studio 2017 and CUDA Toolkit 10.0 in the next release. The 32-bit CUDA and the ATI executables will become unavailable. The latest VS2008 version will be kept in a separate folder. 

A test dataset in TIFF format is also provided in the binary folder. 

<UL>
  <LI>Build-181222. Axis-scan images can be generated by specifying different center values with the same slice position. An alternative robust algorithm for least-square 3D fitting is now implemented. A bugfix in the assembler code for rounding. 
